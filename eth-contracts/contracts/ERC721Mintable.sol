...
contract Pausable is Ownable {
    ...
    event PausedEvent(address caller);
    event UnpausedEvent(address caller);

    modifier whenNotPaused() {
        require(!_paused, 'Cannot execute. Contract is paused.');
        _;
    }

    modifier paused() {
        require(_paused, 'Contract not is paused.');
        _;
    }
...
contract ERC721Mintable is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public {
        require(ownerOf(tokenId) != to, 'The token is already owned by the account');
    ...
    function setApprovalForAll(address to, bool approved) public {
        require(to != msg.sender, 'Operator address must be different from the caller');
